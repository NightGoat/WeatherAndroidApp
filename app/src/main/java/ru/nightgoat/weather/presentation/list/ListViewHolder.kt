...
class ListViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView){
    ...
    fun bind(item: CityEntity, fragment: ListFragmentCallbacks){
        ...
        itemView.listCard_icon.text = fragment.getWeatherIcon(item.iconId, item.date, item.sunrise, item.sunset)
        itemView.listCard_temp.text = item.temp.toString().plus(itemView.context.getString(R.string.degree))
        itemView.setOnClickListener {
            fragment.setCurrentCityAndCallCityFragment(item.name, item.cityId)
            fragment.swapPositionWithFirst(item)
        }
        itemView.listCard.setCardBackgroundColor(ContextCompat.getColor(itemView.context, fragment.getColor(item)))
    }
}